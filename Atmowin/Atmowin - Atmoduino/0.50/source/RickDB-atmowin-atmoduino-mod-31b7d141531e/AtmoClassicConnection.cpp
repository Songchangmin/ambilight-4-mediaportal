...
char *CAtmoClassicConnection::getChannelName(int ch)
{
	...
	double mp = getNumChannels() / 4.0;
	if(mp > 0)ch = (ch / mp) + 1;

	switch(ch) {
	case 0:
		sprintf(buf,"Combined Channel [%d]",ch);
		break;
	case 1:
		sprintf(buf,"Left Channel [%d]",ch);
		break;
	case 2:
		sprintf(buf,"Right Channel [%d]",ch);
		break;
	case 3:
		sprintf(buf,"Top Channel [%d]",ch);
		break;
	case 4:
		sprintf(buf,"Bottom Channel [%d]",ch);
		break;
	default:
