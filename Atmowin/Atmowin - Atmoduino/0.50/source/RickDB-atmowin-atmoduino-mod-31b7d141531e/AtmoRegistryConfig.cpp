...
void CAtmoRegistryConfig::SaveSettings() {
	...
	WriteRegistryInt(configRootKey,"hAtmoClLeds", m_hAtmoClLeds);
...
void CAtmoRegistryConfig::LoadSettings() 
{
	...
	m_hAtmoClLeds      = ReadRegistryInt(configRootKey,"hAtmoClLeds", m_hAtmoClLeds );
...
int CAtmoRegistryConfig::RegistryKeyExists(char *path) {
	HKEY keyHandle;
	if( RegOpenKeyEx(HKEY_LOCAL_MACHINE, path, 0, KEY_READ, &keyHandle) == ERROR_SUCCESS) {
...
int CAtmoRegistryConfig::RegistryValueExists(char *path, char *valueName) {
	HKEY keyHandle;
	int temp = 0;
    DWORD size1;
    DWORD valueType;
...
void CAtmoRegistryConfig::WriteRegistryInt(char *path, char *valueName, int value) {
    HKEY keyHandle;

	if( RegCreateKeyEx(HKEY_LOCAL_MACHINE, path, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &keyHandle, NULL)  == ERROR_SUCCESS) {
...
void CAtmoRegistryConfig::WriteRegistryString(char *path, char *valueName, char *value) {
    HKEY keyHandle;
...
void CAtmoRegistryConfig::WriteRegistryDouble(char *path, char *valueName, double value) {
    HKEY keyHandle;
	if( RegCreateKeyEx(HKEY_LOCAL_MACHINE, path, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &keyHandle, NULL)  == ERROR_SUCCESS) {
...
int CAtmoRegistryConfig::ReadRegistryInt(char *path, char *valueName, int default_value) {
	HKEY keyHandle;
	int tempValue;
    DWORD size1;
    DWORD valueType;
...
char * CAtmoRegistryConfig::ReadRegistryString(char *path, char *valueName, char *default_value) {
	HKEY keyHandle;
	char *tempValue = NULL;
    DWORD size1;
    DWORD valueType;
...
double CAtmoRegistryConfig::ReadRegistryDouble(char *path, char *valueName, double default_value) {
	HKEY keyHandle;
	double tempValue;
    DWORD size1;
    DWORD valueType;
